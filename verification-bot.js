...
client.once('ready', () => {
    ...
    console.log(`📋 Setup: Use -verify in your verify channel to create the panel`);
    console.log(`👥 Once created, EVERYONE can click the button to verify!`);
    console.log(`✏️  Bot will auto-update nicknames with Roblox usernames`);
    console.log(`⚠️  Make sure bot has "Manage Nicknames" permission!`);
});

// ============================================================================
// GHOST PING - New Member Handler
// ============================================================================
...
client.on('guildMemberAdd', async (member) => {
    try {
        ...
        console.log(`[GHOST PING] Pinged ${member.user.tag} in ${ghostPingChannel.name}`);
        
        // Delete the ping after 1 second
        setTimeout(async () => {
            try {
                await pingMessage.delete();
                console.log(`[GHOST PING] Deleted ping for ${member.user.tag}`);
            } catch (error) {
                console.error('[GHOST PING] Failed to delete:', error.message);
            }
        }, 1000);
...
async function createVerificationPanel(channel) {
    const embed = new EmbedBuilder()
        ...
        .setFooter({ text: '👇 Click the green button below to get verified!' })
        .setTimestamp()
...
client.on('messageCreate', async (message) => {
    try {
        ...
        // -verify command (creates the panel) - ADMIN ONLY for setup
        if (content === '-verify') {
            // Only admins can create the panel
            ...
            // Delete the command message for clean look
            try {
    ...
    try {
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId.startsWith('verify_check_')) {
                ...
                // Update Discord nickname to include Roblox username
                try {
                    const currentNick = member.nickname || member.user.username;
                    
                    // Remove old Roblox username if it exists (in parentheses)
                    const cleanNick = currentNick.replace(/\s*\([^)]*\)\s*$/, '').trim();
                    
                    // Add new Roblox username in parentheses
                    const newNick = `${cleanNick} (${robloxUser.name})`;
                    
                    // Only update if under 32 characters (Discord limit)
                    if (newNick.length <= 32) {
                        await member.setNickname(newNick);
                        console.log(`[NICKNAME] Updated ${interaction.user.tag} to "${newNick}"`);
                    } else {
                        // If too long, just use Roblox username
                        await member.setNickname(`(${robloxUser.name})`);
                        console.log(`[NICKNAME] Name too long, set to "(${robloxUser.name})"`);
                    }
                } catch (nickError) {
                ...
                const successEmbed = new EmbedBuilder()
                    ...
                    .setDescription(
                        ...
                        '> **Nickname:** Your name was updated to show your Roblox username!\n\n' +
